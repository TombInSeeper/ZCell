...
static inline void persist_data(int sync, int type, void* data, size_t len)
{
    if (sync) {
        volatile char* buff = (char*)data;
        volatile char* ptr = (char*)((uint64_t)buff & ~(64 - 1));
        for (; ptr < buff + len; ptr += 64) {
            if (type == CLFLUSH_USED) {
                asm_clflush((uint64_t*)ptr);
            } else if (type == CLWB_USED) {
...
static inline void nvmem_memcpy(int sync, char* dest, const char* src, size_t len)
{
